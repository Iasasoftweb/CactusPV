...
uses
  ...
  Vcl.DBCGrids, jpeg, pngimage, GIFImg, vcl.wwbutton, vcl.fcimager, vcl.fcpanel,
  RxExtenders, dxBevel, vcl.wwdbigrd, vcl.wwdbgrid, vcl.wwcheckbox, Vcl.Buttons,
  AdvPanel, AdvPicture, DBAdvPicture, CurvyControls, AdvSmoothPanel, cxTimeEdit,
  dxLayoutContainer, cxGridLayoutView, cxGridDBLayoutView,
  cxGridCustomLayoutView, AdvObj, BaseGrid, AdvGrid, DBAdvGrid;
...
type
  TFrmNFact = class(TForm)
    ...
    cxGrid1DBTableView2ARTICULO: TcxGridDBColumn;
...
//    procedure SaborespanelClick(Sender: TObject);
...
END;
 ...
 procedure TFrmNFact.cxGrid1DBTableView1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  RecIdx: Integer;
...
begin
  ...
  begin
    if (cxGrid1.FocusedView is TcxGridTableView) and (cxGrid1.FocusedView.IsMaster)  Then
      with TcxGridTableView(cxGrid1.FocusedView) do
      begin
        RecIdx := Controller.FocusedRecordIndex;
        TcxGridViewData(cxGrid1.FocusedView.ViewData).Records [RecIdx].Expanded := True;

        DC := DataController.GetDetailDataController(RecIdx, 0);
     //   TcxGridDBDataController(DC).GridView.Focused := True;
        DC.FocusedRowIndex := 0;
      end;
  end;
...
procedure TFrmNFact.fcDBImager3Click(Sender: TObject);
begin
with mantarti22 do
begin
  close;
...
procedure TFrmNFact.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = vk_f2 then
  begin
 // cxDBPivotGrid1.ViewData.Rows[1].GetGroupItemByField(cxDBPivotGrid1Group).Expanded := True;
    //cxGrid1DBTableView1.ViewData.Expand =:true;// DataController.Groups.FullExpand;
  end;
end;
